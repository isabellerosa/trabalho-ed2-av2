...
DicionarioVertices* criaDicionario(){
	DicionarioVertices* dicionario = (DicionarioVertices*) malloc(sizeof(DicionarioVertices));
	dicionario->items = (DicionarioItem*) malloc(sizeof(DicionarioItem)*DICIONARIO_CONST);
...
void map(DicionarioVertices* dicionario, void* vertice){
	...
	if(dicionario->total == dicionario->capacidade){
		dicionario->items = (DicionarioItem*) realloc(dicionario->items, sizeof(DicionarioItem*)*(dicionario->capacidade+DICIONARIO_CONST));
	}
...
void insereCaminho(Grafo* grafo, void* origem, void* destino, int peso) {
	int verticesNovos = 0;
	// mapeia origem e destino caso não estejam mapeados
	...
	if (verticesNovos && grafo->qtdVertices == grafo->dicionario->capacidade){
